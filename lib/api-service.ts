...
import type { ResidentInfo } from "@/lib/supabase/storage-organization-utils";
...
export class ApiService {
  ...
  async uploadAdministrativeDocument(
    ...
    // Get resident info for organized folder structure
    console.log("🔍 Processing upload with metadata:", {
      resident_id: metadata.resident_id,
      resident_badge: metadata.resident_badge,
      resident_name: metadata.resident_name,
      document_type: metadata.document_type
    });

    let residentInfo = null;

    // First try to use provided metadata
    if (metadata.resident_badge && metadata.resident_name) {
      // Parse resident name if available
      const nameParts = metadata.resident_name.split(' ');
      residentInfo = {
        badge: metadata.resident_badge,
        firstName: nameParts[0] || '',
        lastName: nameParts.slice(1).join(' ') || '',
        fullName: metadata.resident_name
      };
      console.log("✅ Using provided resident info from metadata:", residentInfo);
    }
    // If not provided, fetch from database
    else if (metadata.resident_id) {
      console.log("📋 Fetching resident info from database for ID:", metadata.resident_id);
      const { data: resident, error: residentError } = await this.supabase
        .from('residents')
        .select('badge, first_name, last_name')
        .eq('id', metadata.resident_id)
        .single();
      ...
      if (resident) {
        ...
        console.log("✅ Fetched resident info from database:", residentInfo);
      } else {
        console.warn("⚠️ Could not fetch resident info for ID:", metadata.resident_id);
      }
    } else {
      console.warn("⚠️ No resident badge/name or ID provided in metadata");
    }
    ...
    let fileName: string;
    let bucketName = "administrative-documents";

    // Always use organized folder structure when possible
    const { createOrganizedStoragePath, getStorageBucketName } = await import('./supabase/storage-organization-utils');

    // For OUT documents, use the dedicated OUT bucket
    if (metadata.document_type === "OUT") {
      bucketName = "administrative-documents-out";
    }

    if (residentInfo && residentInfo.badge) {
      // Create organized path with individual resident folder
      fileName = createOrganizedStoragePath(residentInfo, sanitizedName, metadata.document_type);
      console.log(`📁 Using organized path with resident folder:`, {
        bucket: bucketName,
        path: fileName,
        residentInfo: residentInfo,
        expectedFolder: `${residentInfo.badge}_${residentInfo.fullName?.replace(/\s+/g, '_')}`
      });
    } else if (metadata.resident_badge || metadata.resident_id) {
      // Try to create minimal organized structure with whatever info we have
      const fallbackInfo: ResidentInfo = {
        badge: metadata.resident_badge || metadata.resident_id?.toString() || 'unknown',
        firstName: '',
        lastName: '',
        fullName: metadata.resident_name || 'Unknown'
      };
      fileName = createOrganizedStoragePath(fallbackInfo, sanitizedName, metadata.document_type);
      console.log(`⚠️ Using fallback organized structure:`, {
        bucket: bucketName,
        path: fileName,
        fallbackInfo
      });
      ...
      fileName = `${metadata.document_type}/${Date.now()}_${sanitizedName}`;
      console.log(`⚠️ No resident info available, using timestamp fallback: ${bucketName}/${fileName}`);
    }
    ...
    if (uploadError) {
      if (uploadError.message?.includes("Bucket not found")) {
        throw new Error(
          `Storage bucket "${bucketName}" not found. Please create it in Supabase dashboard under Storage.`,
        );
      }
      throw uploadError;
    }
    ...
    // Create enhanced document record with organized folder information
    let description = metadata.description;
    if (residentInfo && description) {
      // Enhance description with folder information using utility function
      const { createDocumentDescription } = await import('./supabase/storage-organization-utils');
      description = createDocumentDescription(description, residentInfo);
    }
    ...
    const insertPayload = {
      ...
      file_size: file.size,
      mime_type: file.type,
      // Add enhanced tracking fields for organized structure
      resident_badge: residentInfo?.badge || null,
      resident_name: residentInfo?.fullName || null,
    };
...
export const weekendPermissionsApi = {
      ...
      for (const permission of permissions) {
        try {
          ...
          const dbPermission = {
            ...
            friday_destination: permission.friday.destination || null,
            saturday_type: permission.saturday.type,
            saturday_destination: permission.saturday.destination || null,
            sunday_type: permission.sunday.type,
            sunday_destination: permission.sunday.destination || null,
            monday_type: permission.monday?.type || null,
            monday_destination: permission.monday?.destination || null,
          ...
          // Log destination values for debugging
          if (permission.friday.type === 'none' || permission.saturday.type === 'none' || permission.sunday.type === 'none') {
            console.log(`📤 Saving to DB - Resident ${permission.residentId}:`, {
              friday: { type: dbPermission.friday_type, destination: dbPermission.friday_destination },
              saturday: { type: dbPermission.saturday_type, destination: dbPermission.saturday_destination },
              sunday: { type: dbPermission.sunday_type, destination: dbPermission.sunday_destination }
            });
          }
  ...
  // Delete expired permissions (Monday has passed)
  deleteExpiredPermissions: async () => {
    try {
      const supabase = createClient() as any;

      // Get all unique weeks from the database
      ...
      // Get unique weeks
      const uniqueWeeks = [...new Set(weeks.map((w: any) => w.week))];
      const currentDate = new Date();
      let deletedCount = 0;

      // Check each week and delete if expired
      for (const week of uniqueWeeks) {
        try {
          const [year, weekNum] = (week as string).split('-W');
          ...
          targetThursday.setDate(weekOneThursday.getDate() + (weekNumber - 1) * 7);
          ...
          // If Sunday has passed, delete this week's permissions
          if (currentDate > sunday) {
            ...
            if (deleteError) {
              console.error(`Error deleting week ${week}:`, deleteError);
            } else {
              deletedCount++;
              console.log(`Deleted expired permissions for week ${week}`);
            }
          }
        } catch (weekError) {
      ...
      return {
        ...
        error:
          error instanceof Error
            ? error.message
            : "Failed to delete expired permissions",
      };
    }
  },
