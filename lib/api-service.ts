...
export const activityLogApi = {
  ...
  async getRecent(limit: number = 50): Promise<ApiResponse> {
    return apiCall(`?endpoint=activity-log&limit=${limit}`);
  },
  ...
  async log(activity: {
    ...
    return apiCall('?endpoint=activity-log', {
      method: 'POST',
...
export async function testDatabaseConnection(): Promise<boolean> {
  try {
    const response = await fetch(`${API_BASE_URL}/?endpoint=test-connection`, {
      headers: {
        'X-Api-Key': API_KEY,
      },
    });
...
export async function fetchDashboardStats(): Promise<DashboardStats> {
  try {
    ...
    const residentsResult = await residentsApi.getAll();
    const residents = residentsResult.success && residentsResult.data ? residentsResult.data : [];
    
    // Get room occupancy data
    const roomsResult = await roomsApi.getOccupancy();
    const roomsData = roomsResult.success && roomsResult.data ? roomsResult.data : {};
    
    // Calculate statistics
    const totalResidents = residents.length;
    const occupiedRooms = Object.values(roomsData).filter((room: any) => room && room.residents && room.residents.length > 0).length;
    const totalRooms = Object.keys(roomsData).length || 50; // Default to 50 if no room data
    ...
    try {
      const activityResult = await activityLogApi.getRecent(10);
      if (activityResult.success && activityResult.data) {
        recentActivity = activityResult.data.map((activity: any) => ({
