...
export function DataProvider({ children }: DataProviderProps) {
  ...
  const createResident = async (resident: Omit<Resident, 'id' | 'created_at' | 'updated_at'>): Promise<Resident | null> => {
    try {
      ...
      try {
        const syncResult = await apiService.syncResidentDocuments(resident.badge, newResident.id, 'IN', { residentName: { firstName: newResident.first_name ?? resident.first_name ?? null, lastName: newResident.last_name ?? resident.last_name ?? null } })
  ...
  const addMultipleToDataMatchIt = async (residents: any[]) => {
    ...
    try {
      ...
      for (const importedResident of residents) {
        ...
        if (dbResident) {
          try {
            const syncResult = await apiService.syncResidentDocuments(dbResident.badge, dbResident.id, 'IN', { residentName: { firstName: dbResident.first_name ?? null, lastName: dbResident.last_name ?? null } })
            if (syncResult.synced > 0) {
