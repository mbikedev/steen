...
interface DataContextType {
  ...
  setAgeVerificationStatus: (
    badge: number,
    status: "Meerderjarig" | "Minderjarig" | null,
  ) => void;
  ...
  batchUpdateResidents: (
    updates: Array<{ id: number; updates: Partial<Resident> }>,
  ) => Promise<void>;
...
export function DataProvider({ children }: DataProviderProps) {
  ...
  const [ageVerificationStatus, setAgeVerificationStatus] = useState<
    Record<number, "Meerderjarig" | "Minderjarig" | null>
  >({});
  ...
  const refreshResidents = async () => {
    try {
      ...
      const inResidents = transformedData
        .filter((r: any) => r.status !== "OUT")
        .sort((a: any, b: any) => {
          // Sort by badge number (convert to number for proper sorting)
          const badgeA = parseInt(a.badge) || 0;
          const badgeB = parseInt(b.badge) || 0;
          return badgeA - badgeB;
        });
      const outResidentsFromDB = transformedData
        .filter((r: any) => r.status === "OUT")
        .sort((a: any, b: any) => {
          // Sort OUT residents by badge number too
          const badgeA = parseInt(a.badge) || 0;
          const badgeB = parseInt(b.badge) || 0;
          return badgeA - badgeB;
        });
  ...
  const refreshEssential = async () => {
    ...
    try {
      ...
      if (queueStatus.queueLength > 10) {
        console.warn(
          "⚠️ Request queue is full, clearing to prevent resource exhaustion",
        );
        requestManager.clearQueue();
      }

      // Only load essential data for bed management with controlled requests
      await executeWithResourceControl(
        refreshResidents,
        3,
        "refresh residents",
      );
  ...
  const createResident = async (
    ...
    try {
      ...
      try {
        const syncResult = await apiService.syncResidentDocuments(
          resident.badge,
          newResident.id,
          "IN",
          {
            residentName: {
              firstName: newResident.first_name ?? resident.first_name ?? null,
              lastName: newResident.last_name ?? resident.last_name ?? null,
            },
          },
        );
        if (syncResult.synced > 0) {
          console.log(
            `✅ Synced ${syncResult.synced} documents for resident ${resident.badge}`,
          );
        }
      } catch (syncError) {
        console.error(
          "Failed to sync documents for resident:",
          resident.badge,
          syncError,
        );
      }
      ...
      try {
        ...
        console.error(
          "❌ Failed to refresh dashboard stats after creation:",
          err,
        );
      }
  ...
  const batchUpdateResidents = async (
    updates: Array<{ id: number; updates: Partial<Resident> }>,
  ) => {
    try {
      ...
      console.log(
        `📦 Processing ${updates.length} updates in ${chunks.length} chunks`,
      );
      ...
      for (let i = 0; i < chunks.length; i++) {
        ...
        try {
          ...
          if (failed > 0) {
            console.warn(
              `⚠️ Chunk ${i + 1}/${chunks.length}: ${successful} successful, ${failed} failed`,
            );
          } else {
            console.log(
              `✅ Processed chunk ${i + 1}/${chunks.length}: all ${successful} successful`,
            );
          }
  ...
  const deleteResident = async (id: number) => {
    ...
    try {
      ...
      console.error(
        "❌ Failed to refresh dashboard stats after deletion:",
        err,
      );
  ...
  const createStaffAssignment = async (
    ...
    const newAssignment: StaffAssignment =
      await apiService.createStaffAssignment(assignment);
    await refreshStaffAssignments();
    await apiService.logActivity({
      action: "create",
  ...
  const updateStaffAssignment = async (
    ...
    const updatedAssignment = await apiService.updateStaffAssignment(
      id,
      updates,
    );
    await refreshStaffAssignments();
  ...
  const createPermission = async (
    ...
    const newPermission: Permission =
      await apiService.createPermission(permission);
    await refreshPermissions();
    await apiService.logActivity({
      action: "create",
      ...
      details: {
        ...
        dates: `${permission.start_date} to ${permission.end_date}`,
      },
    });
  ...
  const addToDataMatchIt = async (resident: any): Promise<boolean> => {
    ...
    try {
      ...
      const truncateString = (
        ...
        console.warn(
          `Truncating field from ${str.length} to ${maxLength} characters: "${str.substring(0, 30)}..."`,
        );
      ...
      const supabaseResident: any = {
        ...
        register_number: truncateString(
          resident.registerNumber || resident.register_number,
          100,
        ),
        date_of_birth: resident.dateOfBirth || resident.date_of_birth || null,
        age: resident.age || null,
        gender: resident.gender || null,
        reference_person: truncateString(
          resident.referencePerson || resident.reference_person,
          300,
        ),
  ...
  const addMultipleToDataMatchIt = async (residents: any[]) => {
    console.log(
      `🔄 addMultipleToDataMatchIt called - adding ${residents.length} residents`,
    );
    let successCount = 0;
    let skipCount = 0;
    ...
    console.log(
      `✅ Batch import complete: ${successCount} added, ${skipCount} skipped (duplicates or errors)`,
    );
    ...
    try {
      ...
      for (const importedResident of residents) {
        ...
        const dbResident = allResidents.find(
          (r: any) => r.badge === importedResident.badge,
        );
        if (dbResident) {
          try {
            const syncResult = await apiService.syncResidentDocuments(
              ...
              {
                residentName: {
                  firstName: dbResident.first_name ?? null,
                  lastName: dbResident.last_name ?? null,
                },
              },
            );
            if (syncResult.synced > 0) {
              ...
              console.log(
                `📄 Synced ${syncResult.synced} documents for ${dbResident.badge}`,
              );
            }
          } catch (syncError) {
            console.error(
              `Failed to sync documents for ${dbResident.badge}:`,
              syncError,
            );
          }
        }
  ...
  const updateInDataMatchIt = (id: number, updates: any) => {
    ...
    if (updates.nationality !== undefined) {
      ...
      const autoLanguage = getOfficialLanguageByNationality(
        updates.nationality,
      );
      if (autoLanguage) {
        supabaseUpdates.language = autoLanguage;
        console.log(
          `✅ Auto-filled language "${autoLanguage}" for nationality "${updates.nationality}" (ID: ${id})`,
        );
      }
    }
    if (updates.ovNumber !== undefined)
      supabaseUpdates.ov_number = updates.ovNumber;
    if (updates.registerNumber !== undefined)
      supabaseUpdates.register_number = updates.registerNumber;
    if (updates.dateOfBirth !== undefined)
      supabaseUpdates.date_of_birth = updates.dateOfBirth;
    ...
    if (updates.referencePerson !== undefined)
      supabaseUpdates.reference_person = updates.referencePerson;
    if (updates.dateIn !== undefined) supabaseUpdates.date_in = updates.dateIn;
    if (updates.dateOut !== undefined)
      supabaseUpdates.date_out = updates.dateOut;
    if (updates.daysOfStay !== undefined)
      supabaseUpdates.days_of_stay = updates.daysOfStay;
    if (updates.status !== undefined) supabaseUpdates.status = updates.status;
    if (updates.remarks !== undefined)
      supabaseUpdates.remarks = updates.remarks;
    if (updates.roomRemarks !== undefined)
      supabaseUpdates.room_remarks = updates.roomRemarks;
    if (updates.photoUrl !== undefined)
      supabaseUpdates.photo_url = updates.photoUrl;
  ...
  const deleteFromDataMatchIt = (id: number) => {
    console.log(
      "🔄 deleteFromDataMatchIt called - converting to deleteResident",
    );
  ...
  const deleteMultipleFromDataMatchIt = (ids: number[]) => {
    console.log(
      "🔄 deleteMultipleFromDataMatchIt called - converting to multiple deleteResident calls",
    );
    // Save current state for undo
    setUndoStack((prev) => [...prev, residents]);
    setRedoStack([]);
  ...
  const clearAllData = () => {
    console.log(
      "🔄 clearAllData called - not implemented for Supabase version",
    );
  ...
  const undoDelete = async () => {
    ...
    if (undoStack.length > 0) {
      ...
      try {
        ...
        const deletedResidents = previousState.filter(
          (r: any) => !currentIds.has(r.id),
        );

        if (deletedResidents.length > 0) {
          console.log(
            `🔄 Restoring ${deletedResidents.length} deleted residents`,
          );
          ...
          for (const resident of deletedResidents) {
            ...
            const supabaseResident = {
              ...
              remarks: resident.remarks || null,
              room_remarks:
                resident.roomRemarks || resident.room_remarks || null,
              photo_url: resident.photoUrl || resident.photo_url || null,
            };
  ...
  const setDataMatchIt = (newResidents: any[]) => {
    ...
    newResidents.forEach((resident, index) => {
      if (resident.id) {
        ...
        console.warn(
          "setDataMatchIt: Resident without ID found at index",
          index,
        );
      }
    });
  ...
  const handleSetAgeVerificationStatus = (
    badge: number,
    status: "Meerderjarig" | "Minderjarig" | null,
  ) => {
  ...
  const moveToOutAndDelete = async (residentId: number) => {
    ...
    console.log(
      "👤 Found resident:",
      resident
        ? `${resident.first_name} ${resident.last_name} (Badge: ${resident.badge})`
        : "NOT FOUND",
    );

    if (resident) {
      console.log("✅ Starting comprehensive OUT process");

      try {
        // Step 1: Transfer/Copy documents from IN to OUT bucket
        console.log("📂 Step 1: Transferring documents to OUT bucket");
        const { transferResidentDocumentsToOut } = await import('./supabase/document-transfer-api');
        ...
        if (documentTransferResult.success) {
          console.log(`✅ Documents transferred successfully: ${documentTransferResult.transferredCount} documents`);
          if (documentTransferResult.errors.length > 0) {
            console.warn("⚠️ Some document transfer errors:", documentTransferResult.errors);
          }

          // Log folder cleanup results
          if (documentTransferResult.folderCleanup) {
            const cleanup = documentTransferResult.folderCleanup;
            if (cleanup.attempted) {
              if (cleanup.success) {
                console.log(`🗑️ IN folder cleanup completed: ${cleanup.deletedFiles.length} files deleted`);
                console.log('📁 Deleted files:', cleanup.deletedFiles);
              } else {
                console.warn('⚠️ IN folder cleanup had errors:', cleanup.cleanupErrors);
              }
            } else {
              console.log('ℹ️ IN folder cleanup was skipped');
            }
          }
        } else {
          console.error("❌ Document transfer failed:", documentTransferResult.errors);
          // Continue with the process even if document transfer fails
        }

        // Step 2: Update youth overview status to OUT
        console.log("📊 Step 2: Updating youth overview status");
        try {
          const { moveYouthToTab } = await import('./supabase/youth-overview-api');
          await moveYouthToTab(
            resident.badge.toString(),
            'OUT',
            {
              datum_transfer: new Date().toISOString().split('T')[0],
              out_status: 'Uitgegaan',
              transferdossier_verzonden: 'Documenten overgedragen'
            }
          );
          console.log("✅ Youth overview updated to OUT status");
        } catch (youthOverviewError) {
          console.error("❌ Failed to update youth overview:", youthOverviewError);
          // Continue with the process
        }

        // Step 3: Update resident status in database to 'OUT' - this preserves all data
        console.log("📝 Step 3: Updating resident status in database");
        ...
        // Step 4: Log the status change to database
        console.log("📝 Step 4: Logging status change");
        await apiService.createStatusHistoryEntry({
          ...
          notes: `Resident ${resident.first_name} ${resident.last_name} (Badge: ${resident.badge}) moved to OUT status. Documents: ${documentTransferResult.transferredCount} transferred`,
        });
        ...
        console.log(
          "💾 All resident data, documents, and history preserved in database",
        );
      } catch (error) {
        ...
        bedOccupancy: (() => {
          ...
          return roomStructure.map((roomConfig) => {
            ...
            return {
              ...
              occupancyRate:
                roomConfig.maxBeds > 0
                  ? (roomResidents.length / roomConfig.maxBeds) * 100
                  : 0,
              residents: roomResidents.map((r, index) => ({
                id: r.id,
                name: `${r.first_name} ${r.last_name}`,
                badge: r.badge,
                bedNumber: index + 1, // Assign bed numbers based on order (1, 2, 3, etc.)
              })),
            };
          });
        })(),
