...
export async function POST(request: Request) {
  try {
    ...
    if (existingData) {
      ...
      if (updateResult.error && updateResult.error.message?.includes('color_status')) {
      ...
      // If color column doesn't exist, try without it
      if (insertResult.error && insertResult.error.message?.includes('color_status')) {
        const payloadWithoutColor = { ...payload };
        delete (payloadWithoutColor as any).color_status;
    ...
    if (error) {
      ...
      return NextResponse.json({ error: 'Failed to save grid data', details: error.message }, { status: 500 });
    }
    ...
    return NextResponse.json({ error: 'Failed to save grid data', details: String(error) }, { status: 500 });
  }
}
...
export async function PUT(request: Request) {
  try {
    ...
    // If resident_name is empty, delete the record instead of creating/updating
    if (!payload.resident_name || payload.resident_name.trim() === '') {
      ...
      if (deleteError) {
        ...
        if (!deleteError.message.includes('No rows found')) {
          return NextResponse.json({ error: 'Failed to delete grid data', details: deleteError.message }, { status: 500 });
        }
      }
      
      console.log('Successfully deleted empty record for row:', payload.row_number, 'col:', payload.column_number);
      return NextResponse.json({ message: 'Record deleted', row_number: payload.row_number, column_number: payload.column_number });
    }
    ...
    if (error) {
      console.error('Supabase upsert error:', error);
      console.error('Payload was:', payload);
      return NextResponse.json({ error: 'Failed to update grid data', details: error.message }, { status: 500 });
    }
    ...
    return NextResponse.json({ error: 'Failed to update grid data', details: String(error) }, { status: 500 });
  }
}
  ...
  try {
    ...
    if (residentName) {
      ...
      if (error) {
        ...
        return NextResponse.json({ error: 'Failed to delete grid data by resident name', details: error.message }, { status: 500 });
      }

      console.log('Successfully deleted grid data for resident:', residentName);
      return NextResponse.json({ message: 'Records deleted successfully for resident', resident_name: residentName });
    }

    if (!rowNumber || !columnNumber) {
      return NextResponse.json({ error: 'Either (row_number and column_number) or resident_name is required' }, { status: 400 });
    }
    ...
    if (error) {
      ...
      return NextResponse.json({ error: 'Failed to delete grid data', details: error.message }, { status: 500 });
    }

    console.log('Successfully deleted grid data for row:', rowNumber, 'col:', columnNumber);
    return NextResponse.json({ message: 'Record deleted successfully' });
  } catch (error) {
    console.error('Error deleting grid data:', error);
    return NextResponse.json({ error: 'Failed to delete grid data', details: String(error) }, { status: 500 });
  }
}
