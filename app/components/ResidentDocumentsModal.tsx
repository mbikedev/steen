...
export default function ResidentDocumentsModal({
  ...
  const [documentTransferStatus, setDocumentTransferStatus] = useState<{
    inDocuments: number;
    outDocuments: number;
  ...
  // Load document transfer status
  useEffect(() => {
    const loadTransferStatus = async () => {
      if (resident?.id) {
        try {
          const { getDocumentTransferStatus } = await import('../../lib/supabase/document-transfer-api');
          const status = await getDocumentTransferStatus(resident.id);
          setDocumentTransferStatus(status);
        } catch (error) {
          console.error('Failed to load document transfer status:', error);
        }
      }
    };
    ...
    const loadDocuments = async () => {
      if (resident?.id) {
        try {
          ...
          if (resident.badge) {
            try {
              // Determine document type based on resident status
              const documentType = resident.status === "OUT" ? "OUT" : "IN";
              console.log(`📂 Syncing ${documentType} documents for resident`);
              ...
              // If resident is OUT, also sync any remaining IN documents
              if (resident.status === "OUT") {
                try {
                  await apiService.syncResidentDocuments(
                    resident.badge,
                    resident.id,
                  ...
                  console.error("⚠️ Failed to sync remaining IN documents:", inSyncError);
                }
              }
            } catch (syncError) {
              console.error(
                "⚠️ Failed to sync documents from storage:",
                syncError,
              );
            }
          }
  ...
  const handleFileUpload = async (
    ...
    if (!resident?.id) {
      ...
      alert("Fout: Geen bewoner geselecteerd. Probeer de modal te sluiten en opnieuw te openen.");
      return;
    }

    console.log(`Processing ${files.length} file(s) for resident ID: ${resident.id}`);
    ...
    try {
      ...
      for (const file of Array.from(files)) {
        ...
        // Get the resident name properly - handle both property formats
        const residentFirstName = resident.firstName || resident.first_name || '';
        const residentLastName = resident.lastName || resident.last_name || '';
        const residentFullName = `${residentFirstName} ${residentLastName}`.trim();

        console.log("📋 Resident info for upload:", {
      ...
      setUploadStatus("error");
      setUploadMessage(`Upload fout: ${errorMessage}`);

      // Show user-friendly error message
      if (errorMessage.includes("storage")) {
        alert("Fout: Storage bucket niet gevonden. Controleer of 'administrative-documents' en 'administrative-documents-out' buckets bestaan in Supabase.");
      } else if (errorMessage.includes("auth") || errorMessage.includes("unauthorized")) {
        alert("Fout: U bent niet ingelogd. Log opnieuw in en probeer het nog een keer.");
      } else {
        alert(`Upload fout: ${errorMessage}`);
      }
              ...
              <div className="flex items-center space-x-2 sm:space-x-4 min-w-0 flex-1">
                ...
                <div className="text-primary-foreground min-w-0 flex-1">
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center space-y-1 sm:space-y-0 sm:space-x-2 lg:space-x-4 mt-1">
                    ...
                    {/* Document Transfer Status Indicator */}
                    {documentTransferStatus && (
                      <div className="flex items-center space-x-2">
                        {resident.status === "OUT" && documentTransferStatus.hasTransferred ? (
                          <span className="flex items-center bg-green-500/20 backdrop-blur-sm px-2 py-1 rounded-lg text-green-100 text-xs sm:text-sm">
                            <CheckCircle className="w-3 h-3 sm:w-4 sm:h-4 mr-1" />
                            Documenten overgedragen
                          </span>
                        ) : resident.status === "OUT" && !documentTransferStatus.hasTransferred ? (
                          <span className="flex items-center bg-yellow-500/20 backdrop-blur-sm px-2 py-1 rounded-lg text-yellow-100 text-xs sm:text-sm">
                            <AlertCircle className="w-3 h-3 sm:w-4 sm:h-4 mr-1" />
                            Geen documenten overgedragen
                          </span>
                        ) : (
                          <span className="flex items-center bg-blue-500/20 backdrop-blur-sm px-2 py-1 rounded-lg text-blue-100 text-xs sm:text-sm">
                            <Folder className="w-3 h-3 sm:w-4 sm:h-4 mr-1" />
                            {documentTransferStatus.inDocuments} IN docs
                          </span>
                        )}
                      </div>
                    )}
                ...
                <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-6 space-y-4 sm:space-y-0">
                  ...
                  <div className="flex flex-col sm:flex-row items-stretch sm:items-center space-y-2 sm:space-y-0 sm:space-x-2">
                    ...
                    <button
                      ...
                      className="flex items-center justify-center space-x-2 bg-foreground hover:bg-foreground/90 text-white px-4 py-2 rounded-md transition-colors text-sm disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                      <Upload className="w-4 h-4" />
                      <span className="hidden sm:inline">
                        {uploadStatus === "uploading" ? "Bezig met uploaden..." : "Document Uploaden"}
                      </span>
                      <span className="sm:hidden">{uploadStatus === "uploading" ? "Bezig..." : "Upload"}</span>
                    </button>
                    <input
                      ref={fileInputRef}
                      type="file"
                  ...
                  <div className="text-center py-8 sm:py-12 px-4">
                    ...
                    <button
                      onClick={(e) => {
                        e.preventDefault();
                        e.stopPropagation();
                        console.log("Upload button (empty state) clicked");
                        if (fileInputRef.current) {
                          console.log("File input ref found, triggering click");
                          fileInputRef.current.click();
                        } else {
                      ...
                      disabled={uploadStatus === "uploading"}
                      className="mt-2 px-3 sm:px-4 py-2 bg-foreground text-background rounded-md hover:bg-foreground/90 transition-colors text-sm disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                      <Upload className="w-4 h-4 inline-block mr-2" />
                      <span className="hidden sm:inline">
                        {uploadStatus === "uploading" ? "Bezig met uploaden..." : "Eerste Document Uploaden"}
                      </span>
                      <span className="sm:hidden">{uploadStatus === "uploading" ? "Bezig..." : "Upload Document"}</span>
                    </button>
                  </div>
                )}
              </div>
