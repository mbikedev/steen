...
import { ALL_ROOMS, CAPACITY, type RoomConfig } from "../../../lib/bedConfig";

export default function BedManagementPage() {
  const { bedOccupancy, occupancyStats, refreshAll, refreshEssential } =
    useData();
  ...
  const [filterBuilding, setFilterBuilding] = useState<
    "all" | "noord" | "zuid"
  >("all");
  const [filterFloor, setFilterFloor] = useState<"all" | "ground" | "first">(
    "all",
  );
  ...
  useEffect(() => {
    ...
    if (refreshEssential) {
      console.log(
        "🔄 Bed Management: Loading essential data (residents & rooms)",
      );
  ...
  const filteredRooms = bedOccupancy.filter((room) => {
    ...
    const roomConfig = ALL_ROOMS.find(
      (r: RoomConfig) => r.roomNumber === room.roomNumber,
    );
    const matchesFloor =
      filterFloor === "all" || roomConfig?.floor === filterFloor;
  ...
  const getRoomSpecialization = (roomNumber: string) => {
    ...
    if (roomConfig?.floor === "first" && roomConfig?.building === "noord")
      return "Jongens (1e verdieping)";
    if (roomConfig?.floor === "first" && roomConfig?.building === "zuid")
      return "Jongens (1e verdieping)";
    return "Algemeen";
  };
      ...
      <div className="p-6 bg-background min-h-screen transition-colors">
            ...
            <div className="bg-card rounded-lg shadow border border-border p-4">
              <div className="flex items-center">
                ...
                <div>
                  ...
                  <p className="text-2xl font-bold text-foreground">
                    {isClient ? (occupancyStats?.occupiedBeds ?? 0) : 0}
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-card rounded-lg shadow border border-border p-4">
              <div className="flex items-center">
                ...
                <div>
                  ...
                  <p className="text-2xl font-bold text-foreground">
                    {isClient
                      ? (occupancyStats?.occupancyRate ?? 0).toFixed(1)
                      : "0.0"}
                    %
                  </p>
                </div>
        ...
        {isClient &&
          (() => {
            ...
            const noordRooms = filteredRooms.filter(
              (room) => room.building === "noord",
            );
            const zuidRooms = filteredRooms.filter(
              (room) => room.building === "zuid",
            );
            ...
            const renderRoomCard = (room: any) => {
              ...
              return (
                ...
                >
                  {/* Room Header */}
                  <div
                    className={`px-4 py-3 ${room.building === "noord" ? "bg-foreground" : "bg-foreground"}`}
                  >
                    <div className="flex items-center justify-between">
                      ...
                      <span
                        className={`px-2 py-1 rounded-full text-xs font-medium ${getOccupancyColor(room.occupancyRate || 0)}`}
                      >
                    ...
                    <div className="space-y-2">
                      <div className="flex justify-between text-sm">
                        ...
                        <span className="font-medium text-foreground">
                          {room.occupiedBeds} / {room.maxBeds} bedden
                        </span>
                      </div>
                      <div className="flex justify-between text-sm">
                      ...
                      <div className="flex justify-between text-sm">
                        ...
                        <span className="font-medium text-xs text-foreground">
                          {getRoomSpecialization(room.roomNumber)}
                        </span>
                      </div>
                        ...
                        <div className="w-full bg-muted rounded-full h-2">
                          <div
                            className={`h-2 rounded-full ${
                              (room.occupancyRate || 0) === 0
                                ? "bg-muted-foreground/50"
                                : (room.occupancyRate || 0) < 50
                                  ? "bg-accent"
                                  : (room.occupancyRate || 0) < 80
                                    ? "bg-yellow-500"
                                    : (room.occupancyRate || 0) < 100
                                      ? "bg-accent"
                                      : "bg-destructive"
                            }`}
                    ...
                    <div className="px-4 py-3 bg-muted border-t border-border">
                      ...
                      <div className="space-y-1">
                        {room.residents.map((resident: any, index: number) => (
                          <div
                            key={resident.id || `${room.roomNumber}-${index}`}
                    ...
                    <div className="mb-6">
                      <div className="flex items-center gap-3 mb-2">
                        ...
                        <span className="text-sm text-muted-foreground">
                          ({noordRooms.length} kamer
                          {noordRooms.length !== 1 ? "s" : ""})
                        </span>
                      </div>
                      <div className="text-sm text-muted-foreground">
                        Kamernummers: 1.06 - 1.09 (Begane grond - Jongens), 1.14
                        - 1.19 (Eerste verdieping - Jongens)
                      </div>
                    </div>
                  ...
                  <div>
                    <div className="mb-6">
                      <div className="flex items-center gap-3 mb-2">
                        ...
                        <span className="text-sm text-muted-foreground">
                          ({zuidRooms.length} kamer
                          {zuidRooms.length !== 1 ? "s" : ""})
                        </span>
                      </div>
                      <div className="text-sm text-muted-foreground">
                        Kamernummers: 2.06 - 2.08 (Begane grond - Jongens), 2.14
                        - 2.19 (Eerste verdieping - Jongens + MED)
                      </div>
                    </div>
                    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3">
        ...
        <div className="mt-8 space-y-6">
          ...
          {isClient &&
            (() => {
              const noordRooms = bedOccupancy.filter(
                (room) => room.building === "noord",
              );
              const zuidRooms = bedOccupancy.filter(
                (room) => room.building === "zuid",
              );

              const noordStats = {
                totalBeds: noordRooms.reduce(
                  (sum, room) => sum + room.maxBeds,
                  0,
                ),
                occupiedBeds: noordRooms.reduce(
                  (sum, room) => sum + room.occupiedBeds,
                  0,
                ),
                availableBeds: noordRooms.reduce(
                  (sum, room) => sum + (room.maxBeds - room.occupiedBeds),
                  0,
                ),
                occupancyRate:
                  noordRooms.length > 0
              ...
              const zuidStats = {
                totalBeds: zuidRooms.reduce(
                  (sum, room) => sum + room.maxBeds,
                  0,
                ),
                occupiedBeds: zuidRooms.reduce(
                  (sum, room) => sum + room.occupiedBeds,
                  0,
                ),
                availableBeds: zuidRooms.reduce(
                  (sum, room) => sum + (room.maxBeds - room.occupiedBeds),
                  0,
                ),
                occupancyRate:
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <div className="text-center">
                        <div className="text-2xl font-bold text-purple-600">
                          {noordStats.occupancyRate.toFixed(1)}%
                        </div>
                        <div className="text-sm text-muted-foreground">
                          Bezettingsgraad
                        </div>
                  ...
                  <div className="bg-card rounded-lg shadow border p-6">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <div className="text-center">
                        <div className="text-2xl font-bold text-purple-600">
                          {zuidStats.occupancyRate.toFixed(1)}%
                        </div>
                        <div className="text-sm text-muted-foreground">
                          Bezettingsgraad
                        </div>
                    ...
                    <div className="mt-4 text-xs text-muted-foreground text-center">
                      Kamers: 2.06-2.08 (BG - Jongens), 2.14-2.18 (1e -
                      Jongens), 2.19 (1e - MED)
                    </div>
                  </div>
                </div>
          ...
          <div className="bg-card rounded-lg shadow border border-border p-6">
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-2 mb-6">
              <div className="text-center">
                <div className="text-2xl font-bold text-foreground">
                  {CAPACITY?.noord?.total ?? 0}
                </div>
                <div className="text-sm text-muted-foreground">
                  Noord Capaciteit
                </div>
                <div className="text-xs text-muted-foreground">
                  BG: {CAPACITY?.noord?.ground ?? 0} | 1e:{" "}
                  {CAPACITY?.noord?.first ?? 0}
                </div>
              </div>
              <div className="text-center">
                <div className="text-2xl font-bold text-foreground">
                  {CAPACITY?.zuid?.total ?? 0}
                </div>
                <div className="text-sm text-muted-foreground">
                  Zuid Capaciteit
                </div>
                <div className="text-xs text-muted-foreground">
                  BG: {CAPACITY?.zuid?.ground ?? 0} | 1e:{" "}
                  {CAPACITY?.zuid?.first ?? 0}
                </div>
              </div>
            </div>
          ...
          <div className="flex justify-between items-center">
            <div>
              ...
              <span className="font-medium">
                {isClient ? filteredRooms.length : ALL_ROOMS.length}
              </span>{" "}
              van{" "}
              <span className="font-medium">
                {isClient ? bedOccupancy.length : ALL_ROOMS.length}
              </span>{" "}
              kamers
            </div>
            <div className="text-xs text-gray-500 dark:text-gray-400">
              Laatst bijgewerkt:{" "}
              {isClient ? new Date().toLocaleString("nl-NL") : "---"}
            </div>
          </div>
        </div>
      </div>
