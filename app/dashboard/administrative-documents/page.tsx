...
function AdministrativeDocumentsPageContent() {
  const { bewonerslijst, outResidents, moveToOutAndDelete, deleteResident } = useData();
  ...
  // Handle delete OUT resident with confirmation
  const handleDeleteResident = async (e: React.MouseEvent, resident: any) => {
    e.stopPropagation(); // Prevent row click

    if (confirmingDelete === resident.id) {
      // Second click - actually delete
      try {
        console.log(`🗑️ Deleting OUT resident:`, resident.badge, resident.firstName, resident.lastName);
        await deleteResident(resident.id);
        console.log(`✅ Successfully deleted resident ${resident.badge}`);

        // Reset confirmation state
        setConfirmingDelete(null);
      } catch (error) {
        console.error(`❌ Failed to delete resident ${resident.badge}:`, error);
        alert(`Fout bij verwijderen van bewoner ${resident.badge}. Probeer opnieuw.`);
        setConfirmingDelete(null);
      }
    } else {
      // First click - show confirmation
      setConfirmingDelete(resident.id);
                      ...
                      <tbody className="bg-card divide-y divide-border">
                        {sortedResidents.map((resident, index) => (
                          ...
                          >
                            ...
                            <td className="px-6 py-4 whitespace-nowrap text-sm text-foreground">
                              <button
                                onClick={(e) => handleDeleteResident(e, resident)}
                                className={`inline-flex items-center px-3 py-1 rounded-md text-xs font-medium transition-colors ${
                                  confirmingDelete === resident.id
                                    ? 'bg-destructive text-destructive-foreground hover:bg-destructive/80'
                                    : 'bg-secondary text-secondary-foreground hover:bg-secondary/80'
                                }`}
                                title={
                                  confirmingDelete === resident.id
                                    ? 'Klik opnieuw om te bevestigen'
                                    : 'Bewoner definitief verwijderen'
                                }
                              >
                                <Trash2 className="w-3 h-3 mr-1" />
                                {confirmingDelete === resident.id ? 'Bevestigen?' : 'Verwijderen'}
                              </button>
                            </td>
                          </tr>
                        ))}
