...
function DashboardPage() {
  ...
  const navigationCards = [
    ...
    },
    {
      ...
      description: "Print resident ID badges",
      icon: <CreditCard className="h-6 w-6" />,
      href: "/dashboard/badge-printing",
      color: "from-orange-500 to-amber-600",
      bgColor: "from-orange-50 to-amber-100 dark:to-amber-950/50",
    },
  ];
      ...
      <div className="min-h-screen bg-background p-6">
        ...
        <div className="mb-8 relative overflow-hidden">
          <div
            ...
            style={{
              backgroundImage: "url(/images/steenok.png)",
              backgroundSize: "cover",
              backgroundPosition: "center",
            }}
          >
            <div className="relative z-10 flex flex-col lg:flex-row items-center lg:items-center justify-between gap-6">
              <div className="bg-slate-900/40 backdrop-blur-md rounded-xl p-6 border border-slate-700/50 shadow-xl text-center lg:text-left">
                <div className="flex items-center justify-center lg:justify-start mb-4">
                  <img
                    src="/images/logo_feda.png"
                    alt="Fedasil"
                    className="h-12 sm:h-14 lg:h-16 w-auto opacity-80"
                  />
                </div>
                <h1 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-slate-50 drop-shadow-lg">
                  Welkom Terug
                  {user?.email ? `, ${user.email.split("@")[0]}` : ""}!
                </h1>
                <p className="mt-3 text-base sm:text-lg lg:text-xl text-slate-100 font-semibold drop-shadow-md">
                  OOC Steenokkerzeel Beheer Dashboard
                </p>
              </div>
              <div className="text-center lg:text-right bg-slate-100/95 dark:bg-slate-800/95 backdrop-blur-sm rounded-2xl p-4 sm:p-6 border border-slate-300/50 dark:border-slate-600/50 shadow-xl">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-8">
          {displayStats.map((stat, index) => (
            <div
              key={index}
              className="bg-card rounded-2xl shadow-xl p-6 border border-border hover:shadow-2xl transition-all duration-300"
            >
              <div className="flex flex-col sm:flex-row items-center sm:justify-between text-center sm:text-left">
                ...
                <div
                  className={`order-1 sm:order-2 p-3 bg-gradient-to-br ${
                    stat.color === "emerald"
        ...
        <div className="mb-8">
          <div className="bg-card rounded-3xl shadow-2xl border border-border overflow-hidden">
            <div className="px-8 py-6 border-b border-border bg-muted">
              <h3 className="text-xl sm:text-2xl font-bold text-card-foreground flex items-center justify-center sm:justify-start gap-3">
          ...
          <div className="bg-card rounded-3xl shadow-2xl border border-border overflow-hidden">
            <div className="px-8 py-6 border-b border-border">
              ...
              <p className="text-sm text-muted-foreground mt-1">
                Direct naar belangrijke functies
              </p>
            </div>
            <div className="p-8">
                ...
                {navigationCards.map((card, index) => (
                  ...
                  >
                    <div className="flex items-center space-x-4">
                      <div
                        className={`p-3 bg-gradient-to-br ${card.color} rounded-xl shadow-lg group-hover:shadow-xl transition-all duration-300 group-hover:scale-110 text-primary-foreground`}
                      >
                        {card.icon}
                      </div>
