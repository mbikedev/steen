...
function DataMatchItPageContent() {
  ...
  // Helper function to clear Toewijzingen data for a resident
  const clearToewijzingenForResident = async (residentName: string) => {
    try {
      const response = await fetch(`/api/toewijzingen/grid?resident_name=${encodeURIComponent(residentName)}`, {
        method: 'DELETE',
      });

      if (!response.ok) {
        console.warn('Failed to clear Toewijzingen data for resident:', residentName);
        return;
      }

      const result = await response.json();
      console.log('Successfully cleared Toewijzingen data for resident:', residentName, result);
    } catch (error) {
      console.error('Error clearing Toewijzingen data for resident:', residentName, error);
    }
  };

  const saveEdit = async (residentId: number, field: string) => {
    const key = `${residentId}-${field}`;
    const newValue = editingValues[key];
    
    // Get the current resident data to check for referent changes
    const currentResident = dataMatchIt.find(r => r.id === residentId);
    const oldReferent = currentResident?.referencePerson;
    ...
    // Check if referent is being cleared and clear corresponding Toewijzingen data
    if (field === 'referencePerson' && oldReferent && (!processedValue || processedValue === '')) {
      console.log(`Referent cleared for resident: ${currentResident?.firstName} ${currentResident?.lastName} (was: ${oldReferent})`);
      
      // Clear from Toewijzingen using the resident's full name
      const residentFullName = `${currentResident?.firstName || ''} ${currentResident?.lastName || ''}`.trim();
      if (residentFullName) {
        await clearToewijzingenForResident(residentFullName);
      }
    }
  ...
  const handleKeyPress = (e: React.KeyboardEvent, residentId: number, field: string) => {
    if (e.key === 'Enter') {
      saveEdit(residentId, field).catch(error => console.error('Error saving edit:', error));
        ...
        return (
          <div className="flex items-center gap-1">
            <select
              ...
              onBlur={() => saveEdit(resident.id, field).catch(error => console.error('Error saving edit:', error))}
